...
/**
 * Stateless thin wrapper around OpenRouter's chat-completions endpoint.
 *
 * Every method accepts an optional `modelOverride` so the dual-agent pipeline
 * can talk to two different OpenRouter models from the same client instance
 * (Persona model id vs Factual model id).
 */
class OpenRouterClient private constructor(
    ...
    fun applyOverrides(apiKeyOverride: String, modelOverride: String) {
        ...
        if (modelOverride.isNotBlank())  defaultModel = modelOverride.trim()
    }
    ...
    suspend fun chatCompletion(
        ...
        temperature: Double = 0.80,
        maxTokens: Int = MAX_TOKENS
    ): String = withContext(Dispatchers.IO) {
        val effectiveModel = modelOverride.ifBlank { defaultModel }
        var lastEx: Exception? = null
        for (attempt in 0..MAX_RETRIES) {
            if (attempt > 0) delay(RETRY_DELAYS.getOrElse(attempt - 1) { 5_000L })
            try { return@withContext stream(systemPrompt, messages, jsonMode, effectiveModel, temperature, maxTokens) }
    ...
    suspend fun chatCompletionWithTools(
        ...
        val effectiveModel = modelOverride.ifBlank { defaultModel }
        if (tools.isEmpty()) return@withContext EngineResponse.Text(
            chatCompletion(systemPrompt, messages, jsonMode, effectiveModel, temperature, maxTokens))

        var lastEx: Exception? = null
        for (attempt in 0..MAX_RETRIES) {
            if (attempt > 0) delay(RETRY_DELAYS.getOrElse(attempt - 1) { 5_000L })
            try { return@withContext oneShot(systemPrompt, messages, tools, jsonMode, effectiveModel, temperature, maxTokens) }
            catch (e: kotlinx.coroutines.CancellationException) { throw e }
            catch (e: OpenRouterApiException) { if (e.statusCode in 400..499 && e.statusCode != 429) throw e; lastEx = e }
            catch (e: IOException) { lastEx = e }
        }
    ...
    private fun oneShot(
        ...
        val msgsArr = JSONArray().apply {
            ...
            for (m in messages) {
                if (m.role.value == "tool") {
                    ...
                    put(JSONObject().apply { put("role","tool"); put("tool_call_id", cid); put("content", content) })
                } else put(JSONObject().apply { put("role", m.role.value); put("content", m.content) })
            }
        }
        ...
        val body = JSONObject().apply {
            put("model", modelId); put("messages", msgsArr); put("max_tokens", maxTokens)
            put("temperature", temperature); put("top_p", 0.90); put("stream", false)
        ...
        http.newCall(req).execute().use { resp ->
            ...
            val choice = JSONObject(raw).optJSONArray("choices")?.optJSONObject(0) ?: return EngineResponse.Text("")
    ...
    private suspend fun stream(
        systemPrompt: String, messages: List<ChatMessage>, jsonMode: Boolean,
        ...
        val body = JSONObject().apply {
            put("model", modelId); put("messages", msgsArr); put("max_tokens", maxTokens)
            put("temperature", temperature); put("top_p", 0.90); put("stream", true)
            if (jsonMode) put("response_format", JSONObject().apply { put("type","json_object") })
        }.toString().toRequestBody(CONTENT_TYPE.toMediaType())
